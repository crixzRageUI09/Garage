...
ESX.RegisterServerCallback("crixz:vehiclelist", function(source, cb)
    local ownedCars =  {}
...
RegisterServerEvent("crixz:breakveh")
AddEventHandler("crixz:breakveh", function(plate, state)
    local _src = source
...
ESX.RegisterServerCallback('crixz:returnveh', function (source, cb, Propsvehicle)
	local ownedCars = {}
...
ESX.RegisterServerCallback("crixz:vehiclelistPound", function(source, cb)
	local ownedCars = {}
	local _src = source
	local xPlayer = ESX.GetPlayerFromId(_src)
	MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND `stored` = @stored", {
...
ESX.RegisterServerCallback("crixz:BuyPound", function(source, cb)
	local _src = source
	local xPlayer = ESX.GetPlayerFromId(_src)
	
	if xPlayer.getMoney() >= Config.poundprice then
		xPlayer.removeMoney(Config.poundprice)
		TriggerClientEvent('esx:showAdvancedNotification', _src, 'Banque', 'Conseiller', "Vous avez payé ~g~"..Config.poundprice.."$~s~ pour récuperer votre véhicule", 'CHAR_BANK_MAZE', 1)
		cb(true)
	else 
