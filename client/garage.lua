...
CreateThread(function()
    while true do
        ...
        for k, v in pairs(Config.sortie) do
            ...
            if dist <= 15 then
                interval = 0
                DrawMarker(36, pos[k].x, pos[k].y, pos[k].z, 0.0, 0.0, 0.0, -90,0.0,0.0, 3.0, 3.0, 3.0, 47, 232, 64, 255, false, false, p19, false)
            end
            ...
            if dist <= 2 then
                ...
                if IsControlJustPressed(1, 51) then
                    ESX.TriggerServerCallback("crixz:vehiclelist", function(ownedCars)
        ...
        for k, v in pairs(Config.rentrer) do
            ...
            if dist <= 15 then
                interval = 0
                DrawMarker(36, pos[k].x, pos[k].y, pos[k].z, 0.0, 0.0, 0.0, -90,0.0,0.0, 3.0, 3.0, 3.0, 190, 0, 0, 255, false, false, p19, false)
            end
...
function SpawnVehicle(vehicle, plate)
    ...
    TriggerServerEvent("crixz:breakveh", plate, false)
end
...
function ReturnVeh()
    ...
    if IsPedInAnyVehicle(playerPed, false) then 
        ...
        ESX.TriggerServerCallback("crixz:returnveh", function(valid)
            if valid then 
...
function BreakReturnVehicle(vehicle, Propsvehicle)
	...
	TriggerServerEvent('crixz:breakveh', Propsvehicle.plate, true)
